...
- 增加中英双语 Web 控制台：可查看服务状态、缓存配置、用量趋势、public key，并直接创建、编辑或删除缓存。侧边栏 Globe 按钮一键切换语言。
- 管理员 Token 在前端公开显示：这个 LPK 的路由已设置为公开访问，适合用户明确要求"不经过懒猫登录保护"的部署方式。
- 增加独立中英双语教程页：访问 `/guide` 查看创建缓存、客户端登录、推送 store path、Nix/NixOS 配置和排错步骤。
...
- 下载访问计数：每次 NAR 请求递增对象 `access_count`，通过 `/cache` 页面折线图展示用量趋势。
...
缓存详情页位于 `/cache?name=<缓存名>`，以普通用户视角展示：

- 概览卡片：对象数量、公开/私有状态、保留策略、最后上传时间。
- 总下载次数和总存储量。
- 用量趋势折线图：近 30 天每日上传数量/对象活跃趋势（SVG 渲染，shadcn 主题）。
- 系统架构和压缩方式分布。
- 缓存配置编辑，包括公开状态、store 目录、优先级、上游 key 和 retention period。
- 连接信息：API Endpoint、Substituter、Public key、Upstream keys，支持一键复制。
- 删除缓存。

每次通过 Nix Binary Cache API 下载 NAR 时，会自动递增对应对象的访问计数，用于统计总下载次数。
